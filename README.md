...
**It remembers what cannot be hit.** When the client reports a DoT or bleed as *immune*, the
mob's name goes into a small table shared by the whole account (`/sbr immune` lists it), and
that spell is not sent to that kind of mob again — on any character, in any later session.
Only the DoTs and bleeds the rotation sends itself are learned; weapon poisons and procs are
not. A boss shield phase, where everything is immune at once, is recognised and not recorded.
`/sbr immune forget <name>` corrects a wrong entry.
...
- **Talent-Aware Strikes:** Two toggles pick which strikes you use — *Holy Strike* and *Crusader Strike*. Enable one alone to use **only** that strike; enable both to reveal a **strategy** dropdown. **Auto DPS** reads your spec: without *Vengeful Strikes*, *Crusader Strike* builds *Zeal* to 3 stacks and *Holy Strike* fills (still returning mana/health to the group); with the talent, *Holy Strike* opens for *Holy Might* and is kept up while *Zeal* is ramped — if both buffs would fall in the same 6s window, *Zeal* wins. **Tank block** keeps *Crusader Strike*'s *Zealous Defense* block buff loaded (consumed on the next block) and spends every other global on *Holy Strike* for threat (*Righteous Strikes*). **HS, then alternate** opens every fight with *Holy Strike* and then strictly alternates the two. Every tab of the paladin panel keeps its own settings. Also `/sbr strike off|hs|cs|auto|tank|alternate` for a mid-fight keybind.
- **Mana Downranking (opt-in):** *Downrank when low* casts lower ranks of your strikes as raw mana drops, to keep swinging while leveling. Thresholds use absolute mana (not percent), so a full pool stays at top rank and only a near-empty pool steps down, always clamped to your highest known rank: the strike goes out at the highest rank the mana pays for. On the Solofarming tab the switch reads **Crusader Strike at rank 1** and does exactly that — there the strike only resets Holy Shock, and the reset keeps trying until Holy Shock is back, also while you are below its line.
...
- **Whirlwind leads in AoE:** Checked ahead of the primary strike while `/sbr aoe` is on, since it hits everything in range where *Mortal Strike* hits one. The single-target rage dump keeps its usual place; *Rend* and *Sunder Armor* stand down in AoE. **Auto AoE** (off by default) flips the switch from the drawn enemy count in Whirlwind range (slider, default 2; needs nameplates), `/sbr aoe` overrides it press by press and `/sbr run single|aoe` outranks both. **Respect CC** (on) stands AoE down while a Polymorph, Freezing Trap or Sap is on any enemy in the pack — the pack is read through ClassicAPI, without it only your target counts.
...
- **Spec tabs (Starter · Assassination · Combat · Subtlety):** The panel is split by spec, and every tab keeps **its own settings** — a switch flipped on one tab stays on that tab. The Subtlety tab also selects the Subtlety rotation below; the others share the Assassination/Combat engine.
- **Subtlety — the raid rotation (v1.2.35):** Written to what an experienced Subtlety rogue plays and matched against logs line by line: Garrote, Hemorrhage to five, **Expose Armor**, Slice and Dice with the returned point, Hemorrhage to five, **Rupture** for Taste for Blood, Slice and Dice, then the burst — **Mark for Death, Vanish, Garrote, Shadow of Death**, **Preparation**, and the burst again — and Expose Armor, Slice and Dice, Rupture and the sigil on cooldown around it. **Expose Armor is kept up**: its time left is read off the target (ClassicAPI) or counted from the last cast, it is re-applied under the **Expose Armor refresh under** slider, the five points are reserved for it in time (from the energy pooled and the fight's measured income), and a missed cast is recognised and re-done. Nothing is sent into the global cooldown. Five points never idle at full energy: Eviscerate, an early Rupture, or a Hemorrhage take them. Switches: *Garrote from stealth*, *Vanish into the burst*, *Eviscerate when nothing is due*; `/sbr new <name> subtlety` makes a profile with all of it set. Garrote is skipped on Mechanical and Elemental targets and on any mob the immunity table remembers as bleed-immune (`/sbr immune`); Hemorrhage opens there. Needs Flash Powder for Vanish; Thistle Tea stays manual.
...
- **Execute Low-HP Targets (opt-in, off by default):** Below a configurable health threshold (default 10%), *Eviscerate* fires with whatever combo points are on hand rather than waiting for your normal threshold. Two optional conditions narrow it: **Use from** sets a combo-point floor (unspent points cost nothing when the target dies with them, while a 1-point *Eviscerate* costs a full finisher's energy for less damage than the builder it displaces), and **Skip if alive past** cancels the dump when the target is measurably going to outlive that many seconds — an elite parked at 8% no longer collects weak finishers. Applies on the Subtlety tab as well, ahead of Expose Armor. **Rupture — Only if the target lives** (off by default) skips Rupture on a target the time-to-kill estimate says dies sooner than the slider; the points go into *Eviscerate* instead — for raid trash. Adds `hp=`, `en=`, `ttk=`, `exec=` and `cap=` fields to `/sbr trace`.
...
<details>
<summary><b>🏹 Hunter</b> — Beast Mastery / Marksmanship / Survival tabs, reworked for Turtle's Survival</summary>

Reworked for Turtle WoW 1.18.1's hunter changes. The panel has a tab per spec — **Beast Mastery**, **Marksmanship**, **Survival** — each with its own settings over a shared base, and the active tab is the spec the rotation plays (`/sbr spec bm|mm|survival`). Every attack switch has a **SINGLE** and an **AOE** column; `/sbr run single` and `/sbr run aoe` pick the column per press, a bare `/sbr` follows the profile's AoE toggle.
...
* **Switch attacks by distance:** On by default per spec. The spec sets the default range (the two shooting specs at range, Survival in melee) and distance overrides it press by press (a short stickiness stops it flickering at the boundary): a shooting spec that has been closed on uses its *Melee, when closed on* set, a Survival hunter kept at range uses *Ranged, when out of reach*.
...
* **Pet Support:** Pet attack, *Mend Pet* below a health slider, **Kill Command** (only usable after *you* land a critical strike — it is armed by a crit of yours, sent off the global cooldown, and a send consumes it), **Bestial Wrath** on its own switch — it grants the pet *Scent of Blood* for 18 seconds, that talent's own +40% pet damage proc, so it is skipped entirely without a live pet — an optional **Baited Shot** fired in the window after the pet crits, and an optional **Smart Pet Taunt** — when the mob peels onto you, the pet's *Growl* is sent to grab it back (off by default; leave it off for melee-weave builds where you want the aggro).
* **AoE & Cooldowns:** *Multi-Shot* leads on an AoE press, then *Volley* wherever its column is on. Volley lands **under the mouse** and is only sent while the mouse rests on an enemy — hold it on the feet of the pack when pressing (see *Ground-targeted spells* under [A few honest notes](#a-few-honest-notes)). *Rapid Fire* (ranged only) and *Bestial Wrath* (Beast Mastery) automate on the usual three-state model — always, elite/boss only, or off.

> **Verification note:** *Kill Command* and **Lock and Load** are confirmed against the client's own talent data. *Baited Shot* is not — it appears nowhere in the hunter talent tree, which proves nothing on its own (a trainer ability would not be there either) but leaves it the one name here without a source. It is gated by `KnowsSpell`, so a wrong name simply no-ops. If it is not firing, run `/sbr debug` and check the exact name — they drop into one place in `Class_Hunter.lua`. The mana aspect is *Aspect of the Viper* and nothing else: *Aspect of the Beast* was listed here as a second guess and is a different spell entirely — it makes you untrackable and returns no mana — which from level 32 made it the aspect the rotation reached for whenever mana ran low. The distance switch uses `CheckInteractDistance` (~10yd) as its melee proxy; `/sbr trace` shows the spec and the branch taken as `spec=bm/ranged` or `spec=surv/melee`.
</details>
...
* **Balance / Eclipse Weaving:** Keeps *Moonfire* and *Insect Swarm* up, then chain-casts your primary nuke (Wrath or Starfire) to fish for **Eclipse** procs and swaps to the empowered opposite nuke the instant one fires. Nukes are queued through SuperWoW, so spamming never clips a cast. Entering Moonkin (when learned) is automatic for the mana discount. **Hurricane at the mouse (AoE)** (off by default) channels *Hurricane* ahead of the nukes on an AoE press or with the AoE toggle — under the mouse, only while the mouse rests on an enemy; presses during the channel do nothing.
...
* **Learned Bleed Immunity:** *Rip* and *Rake* are not retried forever on a target that cannot bleed. The creature-type test covers the two always-immune types and only in English; the combat log covers the rest, filed by mob name in the account-wide immunity table (`/sbr immune`) and remembered between sessions and characters.
...
* **Rain of Fire on an AoE press:** `/sbr run aoe` (or the profile's AoE toggle) channels *Rain of Fire* below the survival steps and above everything single-target — **under the mouse**, and only while the mouse rests on an enemy; hold it on the feet of the pack. Not while moving, like every channel. On by default, its own switch under *Area (AoE press)*.
* **Movement aware:** The client has no speed API, so position is sampled and differenced (via SuperWoW), five times a second on its own timer — and answers *"standing still"* whenever it cannot tell, so it never blocks a cast on a guess. The detection can be switched off (`/sbr move off`, or the minimap panel) for a client that does not report movement cleanly; then everything is attempted and the client decides. **Channels are not attempted while moving** (*Drain Life*, *Drain Soul*, *Dark Harvest*, *Health Funnel*): movement breaks a channel outright, so starting one is a global cooldown spent on nothing. **DoTs with a cast time are skipped while moving** and the chain carries on to the instant ones — cast times count the talents, so *Corruption* is instant at 5/5 *Improved Corruption* and is used while running, while *Immolate* is not. Standing still, the full rotation runs unchanged.
...
* **Life Tap & Low-Mana Safety:** Triggers *Life Tap* only when mana dips below your threshold **and** the health the tap *leaves* stays above your floor — **Keep HP above with aggro** (default 70%) is the floor while a mob is on you. With nothing attacking you, a *Drain Life* is preceded by a tap while the mana bar has room, so the channel heals back health that would otherwise go to waste. **Drain Mana from casters** (off by default) drains a target that has mana instead of tapping below that same line — mobs without mana are never drained. **Tap out of combat** (off) Life Taps with no target up to a mana percent of your choosing before the next pull, and holds auto-targeting while it does. A separate **Stop Immolate below** line (0 = off) keeps the two-second cast off a dying mob independently of the general DoT stop. A **spell lock** (interrupted mid-cast, the school on a cooldown of several seconds) drops the rotation to the wand until it ends. A separate, lower mana floor is a last-resort safety valve — below it, a DoT that needs recasting but can't be afforded no longer gets queued and left to fail; the rotation prefers Life Tap if safe, otherwise drops to the wand.
...
* **Spirit Tap Finisher:** under a configurable target-health %, the rotation bursts with *Pain Spike* (an instant whose damage heals itself back, so only ever a killing blow), then *Mind Blast*, then *Smite* to **secure the killing blow** — and the experience that feeds *Spirit Tap*.
...
* **AoE Mode (`/sbr aoe`, or per press `/sbr run aoe`):** kite-AoE — *Frost Nova* to freeze, then with the **Blizzard / Flamestrike at the mouse** switch on the ground spell of the spec (*Flamestrike* for fire), *Cone of Cold* to snare, *Icicles*, then *Arcane Explosion* to finish. The ground spell lands **under the mouse** and is only sent while the mouse rests on an enemy — hold it on the feet of the pack. *Evocation* restores mana when low, and channels (*Arcane Missiles*, *Icicles*, *Blizzard*, *Evocation*) are never clipped.

> **Verification note:** Turtle's custom spells were confirmed by exact name against the client spell DB (*Icicles*, *Arcane Rupture*, *Arcane Surge*, *Flash Freeze*, *Fire Vulnerability*), but their **proc / stack behaviour is best-effort** — confirm with `/sbr debug` if something isn't firing. **Ground-targeted AoE (*Blizzard*, *Flamestrike*) lands under the mouse**, never at the target — see [A few honest notes](#a-few-honest-notes).
</details>
...
| `/sbr run single\|aoe` | Fire the rotation reading the single or the AoE column for this press (two macros); a bare `/sbr` follows the profile's AoE toggle |
| `/sbr move on\|off` | Movement detection on or off (off: everything is attempted, the client decides) |
| `/sbr deps` | Which client components are present, with the SuperWoW version |
| `/sbr immune [forget <name>\|clear]` | The learned immunity table: list it, drop one mob (or the current target), or empty it |

### Per class

| Command | Class | What it does |
|---|---|---|
| `/sbr spec <…>` | Hunter | Spec tab — `bm/mm/survival` (`/sbr mode` is an alias) |
| `/sbr mode <…>` | Shaman · Mage | Playstyle/spec — `enhancement/elemental/tank/resto` · `frost/fire/arcane` |
| `/sbr aoe` | Warrior · Paladin · Druid · Hunter · Mage | Toggle AoE mode (per press instead: `/sbr run aoe`) |
...
| `/sbr strike <off/hs/cs/auto/tank/alternate>` | Paladin | Set which strikes are used |
...
- **Ground-targeted spells land under the mouse** (Volley, Blizzard, Flamestrike, Rain of Fire,
  Hurricane). Nothing on this client can place them at the target; the rotation uses
  Nampower's quickcast for the one cast and only sends the spell while the mouse rests on an
  enemy — hold it on the feet of the pack when pressing. Without Nampower they stay manual.
