# Aegis: Single Button Rotation (v1.2.34)
...
- **Talent-Aware Strikes:** Two toggles pick which strikes you use — *Holy Strike* and *Crusader Strike*. Enable one alone to use **only** that strike; enable both to reveal a **strategy** dropdown. **Auto DPS** reads your spec: without *Vengeful Strikes*, *Crusader Strike* builds *Zeal* to 3 stacks and *Holy Strike* fills (still returning mana/health to the group); with the talent, *Holy Strike* opens for *Holy Might* and is kept up while *Zeal* is ramped — if both buffs would fall in the same 6s window, *Zeal* wins. **Tank block** keeps *Crusader Strike*'s *Zealous Defense* block buff loaded (consumed on the next block) and spends every other global on *Holy Strike* for threat (*Righteous Strikes*). **HS, then alternate** opens every fight with *Holy Strike* and then strictly alternates the two. Also `/sbr strike off|hs|cs|auto|tank|alternate` for a mid-fight keybind.
